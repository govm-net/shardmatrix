...
func main() {
	...
	// 获取初始区块链状态
	chainState1 := n1.Blockchain().GetChainState()
	fmt.Printf("Initial blockchain height: %d\n", chainState1.Height)
	fmt.Printf("Initial best block hash: %s\n", chainState1.BestBlockHash.String())
	...
	// 获取第二次启动后的区块链状态
	chainState2 := n2.Blockchain().GetChainState()
	fmt.Printf("Second start blockchain height: %d\n", chainState2.Height)
	fmt.Printf("Second start best block hash: %s\n", chainState2.BestBlockHash.String())
	
	// 验证数据是否一致
	if chainState1.Height == chainState2.Height && 
	   chainState1.BestBlockHash.Equal(chainState2.BestBlockHash) {
		fmt.Println("✅ Data persistence test PASSED: Blockchain state is consistent!")
	} else {
		fmt.Println("❌ Data persistence test FAILED: Blockchain state is inconsistent!")
		fmt.Printf("  First height: %d, Second height: %d\n", chainState1.Height, chainState2.Height)
		fmt.Printf("  First hash: %s, Second hash: %s\n", 
			chainState1.BestBlockHash.String(), chainState2.BestBlockHash.String())
	}
